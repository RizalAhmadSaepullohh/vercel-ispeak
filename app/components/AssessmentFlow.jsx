...
export default function AssessmentFlow() {
  const [step, setStep] = useState(0); // 0=form, 1=task, 7=finish
  ...
  useEffect(() => {
    async function upload() {
      ...
      try {
        setLoading(true);
        const blob = new Blob(chunks, { type: "audio/wav" }); // Use wav to match user screenshot/streamlit
        const file = new File([blob], `recording.wav`, { type: "audio/wav" });
        const fd = new FormData();
        fd.set("mahasiswa_id", String(mahasiswa.id));
        fd.set("name", mahasiswa.name || "participant");
        ...
        if (!res.ok) {
          ...
          throw new Error(jErr?.error || "Upload rekaman gagal");
        }
        const j = await res.json();
        ...
        // Keep only the LATEST recording for this task to avoid duplicates in Step 1/1 flow
        setUploaded((arr) => {
          const filtered = arr.filter(x => x?.tugas?.id !== currentTugas.id);
          return [...filtered, { tugas: currentTugas, rekaman: j?.rekaman, file, refTopic, stepIndex: step }];
        });
        reset();
        setRecordReady(false);
      } catch (e) {
    ...
    if (!isRecording && chunks.length > 0 && recordReady && currentTugas && !alreadyUploaded) {
      upload();
    }
  ...
  const onSubmitMahasiswa = useCallback(async (e) => {
    ...
    if (!perception) return setError("Perception is required");
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(j?.error || "Gagal simpan partisipan");
      }
      const j = await res.json();
  ...
  }, [name, programStudy, age, gender, city, residence, campus, testType, testScore, perception]);
  ...
  const runScoring = useCallback(async () => {
    ...
    try {
      ...
      if (itemsToCompute.length === 0) {
        setError("Tidak ada rekaman untuk Task 1. Silakan rekam Task 1 dulu.");
        return;
      }
      ...
      for (let k = 0; k < itemsToCompute.length; k++) {
        ...
        // We continue even if it's a temp ID
        if (!recId) continue;
        setCurrentIdx(i);
  ...
  const res = await featureRef.current.compute(item.file, refText);
        ...
        try {
          if (item?.stepIndex === 1 && res?.transcript) {
    ...
    <div>
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-4">
          ...
          {mahasiswa && <div className="text-sm text-gray-600">{mahasiswa?.name} · Step {Math.min(step,1)}/1</div>}
        </div>
        {/* Progress bar for steps */}
        <div className="w-full h-2 bg-gray-200 rounded-full mb-4 overflow-hidden">
          <div
            className="h-full bg-black transition-all"
            style={{ width: `${Math.min(step,1) / 1 * 100}%` }}
          />
        </div>
          ...
          <form onSubmit={onSubmitMahasiswa} className="space-y-4 bg-white/70 backdrop-blur rounded-xl shadow p-6 border border-gray-100">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <label className="block">
                ...
                <input required value={name} onChange={(e)=>setName(e.target.value)} className="mt-1 w-full border border-gray-300 focus:border-black focus:ring-black p-2 rounded" placeholder="Your full name" />
              </label>
              <label className="block">
                <span className="text-sm">Major / Program</span>
                <input required value={programStudy} onChange={(e)=>setProgramStudy(e.target.value)} className="mt-1 w-full border border-gray-300 focus:border-black focus:ring-black p-2 rounded" placeholder="e.g., Informatics" />
              </label>
              <label className="block">
                <span className="text-sm">Age</span>
                <input required type="number" min={1} value={age} onChange={(e)=>setAge(e.target.value)} className="mt-1 w-full border border-gray-300 focus:border-black focus:ring-black p-2 rounded" />
              </label>
              <label className="block">
                <span className="text-sm">Gender *</span>
                <select required value={gender} onChange={(e)=>setGender(e.target.value)} className="mt-1 w-full border border-gray-300 focus:border-black focus:ring-black p-2 rounded">
              ...
              <label className="block">
                ...
                <input required value={city} onChange={(e)=>setCity(e.target.value)} className="mt-1 w-full border border-gray-300 focus:border-black focus:ring-black p-2 rounded" placeholder="City" />
              </label>
              <label className="block">
                <span className="text-sm">Current Residence</span>
                <input required value={residence} onChange={(e)=>setResidence(e.target.value)} className="mt-1 w-full border border-gray-300 focus:border-black focus:ring-black p-2 rounded" placeholder="Current Residence" />
              </label>
              <label className="block">
                <span className="text-sm">Campus</span>
                <input required value={campus} onChange={(e)=>setCampus(e.target.value)} className="mt-1 w-full border border-gray-300 focus:border-black focus:ring-black p-2 rounded" placeholder="Campus Name" />
              </label>
              <label className="block">
                <span className="text-sm">Test Type *</span>
                <select required value={testType} onChange={(e)=>setTestType(e.target.value)} className="mt-1 w-full border border-gray-300 focus:border-black focus:ring-black p-2 rounded">
              ...
              <label className="block">
                <span className="text-sm">Test Score (Optional)</span>
                <input type="number" min={0} value={testScore} onChange={(e)=>setTestScore(e.target.value)} className="mt-1 w-full border border-gray-300 focus:border-black focus:ring-black p-2 rounded" />
              </label>
              <label className="block md:col-span-2">
                <span className="text-sm">Perception *</span>
                <select required value={perception} onChange={(e)=>setPerception(e.target.value)} className="mt-1 w-full border border-gray-300 focus:border-black focus:ring-black p-2 rounded">
        ...
        {step === 1 && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="text-sm text-gray-600">Student: {mahasiswa?.name} • Step {step}/1</div>
                ...
                <div className="mt-4 flex flex-col gap-4">
                  <div className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-gray-500">Auto-starts after prep</span>
                    </div>
                  ...
                  {isRecording && (
                    <button 
                      ...
                      className="w-full py-3 rounded-lg bg-red-600 hover:bg-red-700 text-white font-semibold shadow-sm transition-colors flex items-center justify-center gap-2"
                    >
            ...
            <div className="flex items-center gap-2">
              <button onClick={nextStep} disabled={!canNext} className="rounded-lg bg-black hover:bg-neutral-800 transition text-white px-5 py-2.5 shadow">
                {step < 1 ? (loading ? "Uploading..." : "Next Task") : "Finish"}
              </button>
            </div>
          </div>
        )}
        ...
        {step === 7 && (
          <div className="space-y-4">
            ...
            {(() => {
              ...
              const res = rec1 ? scoreResults[rec1] : null;
            ...
            <div className="flex items-center gap-2">
              {(() => {
                const task1 = uploaded.find((u) => u?.stepIndex === 1);
                const rec1 = task1?.rekaman?.id;
                const done1 = rec1 ? scoreDone.includes(rec1) : false;
                if (done1 && !scoring) {
                  return (
                    <button onClick={() => { 
                      // reset to start
